...
const CourseProgress = (() => {
  ...
  function getDefaultState() {
    return {
      ...
      highestCombo: 0,
      submissionAttempts: [],  // [{ts, classId, exNum, ok, http, errorKind, errorMsg}] — diagnóstico de entregas
    };
  }
  ...
  // ============================================
  //  ENTREGA DE EJERCICIOS — método ficheros
  //  Cada entrega es un fichero markdown en
  //  entregas/{classId}/ej{N}.md, escrito vía
  //  Contents API (PUT con sha si re-entrega).
  //  Frontmatter YAML lleva status, type, xpAwarded.
  //  La scheduled task `revision-ejercicios-juan`
  //  reescribe el mismo fichero con feedback y
  //  status=aprobado|revisado.
  // ============================================

  const ENTREGAS_DIR = 'entregas';
  const SUBMISSION_ATTEMPTS_MAX = 25;

  // Categorías estables de error HTTP para diagnóstico sin F12
  const HTTP_ERROR_KIND = {
    401: 'token_invalido',
    409: 'conflicto_sha',
    422: 'validacion_github',
  };
  function _classifyHttpError(status, bodyText) {
    if (status === 401) return { kind: 'token_invalido', userMsg: 'Tu token de GitHub ya no es válido (401). Reconfigura la sincronización en el dashboard.' };
    if (status === 403) {
      const lower = (bodyText || '').toLowerCase();
      if (lower.includes('rate limit') || lower.includes('api rate')) {
        return { kind: 'rate_limit', userMsg: 'GitHub limitó nuestras peticiones (403 rate limit). Espera 5 minutos y reintenta.' };
      }
      return { kind: 'sin_permiso', userMsg: 'El token no tiene permiso de escritura sobre este repo (403). Verifica scopes en GitHub → Settings → Tokens.' };
    }
    if (status === 409) return { kind: 'conflicto_sha', userMsg: 'Conflicto de versión (409): otra escritura cambió el fichero. Recarga la página y reintenta.' };
    if (status === 422) return { kind: 'validacion_github', userMsg: 'GitHub rechazó la entrega (422). Probablemente el fichero está mal formado. Avisa a Cristina.' };
    if (status >= 500) return { kind: 'github_caido', userMsg: `GitHub está dando ${status}. No es problema tuyo, reintenta en 1-2 min.` };
    return { kind: 'otro', userMsg: `Error HTTP ${status}. Detalles en consola. Avisa a Cristina si persiste.` };
  }

  // --- Codificación UTF-8 → base64 (segura para emojis y acentos) ---
  ...
  // --- Persistir caja roja de error bajo el botón (no se va sola) ---
  function _renderSubmitErrorBox(exerciseNum, kind, msg, httpStatus) {
    const el = document.getElementById(`submit-status-ejercicio-${exerciseNum}`);
    if (!el) return;
    const ts = new Date().toLocaleTimeString('es-ES');
    el.innerHTML = `
      <div style="background:rgba(255,82,82,0.12);border:1px solid #ff5252;border-radius:8px;padding:10px 12px;color:#ff8a8a;font-size:0.85em;line-height:1.45;">
        <strong style="color:#ff5252;">❌ No se pudo entregar</strong> · <span style="opacity:0.7;">${ts}</span><br>
        <span style="color:#ffbcbc;">${msg}</span><br>
        <span style="opacity:0.6;font-size:0.78em;">Tipo: <code>${kind}</code> · HTTP ${httpStatus || '—'}</span>
      </div>`;
    el.style.color = '';
  }

  // --- Registro de intentos (éxito o fallo) — campos compatibles con index.html ---
  function recordSubmissionAttempt(classId, exNum, ok, httpStatus, errorKind, errorMsg) {
    try {
      const state = load();
      if (!Array.isArray(state.submissionAttempts)) state.submissionAttempts = [];
      state.submissionAttempts.unshift({
        ts: new Date().toISOString(),
        classId,
        ex: exNum,
        ok: !!ok,
        http: httpStatus || null,
        code: errorKind || (ok ? 'ok' : 'error'),
        msg: errorMsg || null,
      });
      // Cap
      if (state.submissionAttempts.length > SUBMISSION_ATTEMPTS_MAX) {
        state.submissionAttempts = state.submissionAttempts.slice(0, SUBMISSION_ATTEMPTS_MAX);
      }
      save(state);
      // En fallo, forzamos sync inmediato para que Cristina vea el intento desde otra pestaña
      if (!ok) {
        try { syncToGitHub(); } catch (e) { /* sync best-effort */ }
      }
    } catch (e) {
      console.warn('recordSubmissionAttempt: no se pudo persistir intento', e);
    }
  }

  // --- Entrega: PUT a entregas/{classId}/ej{N}.md ---
  async function submitExercise(classId, exerciseNum, buttonEl) {
    ...
    const classNum = classPart.replace('c', '').replace('bb', 'bb');
    ...
    // Construir frontmatter YAML + cuerpo markdown
    const nowIso = new Date().toISOString();
    const path = `${ENTREGAS_DIR}/${classId}/ej${exerciseNum}.md`;
    const yamlEsc = (s) => String(s == null ? '' : s).replace(/"/g, '\\"');
    ...
    const fmLines = [
      ...
      `type: ${exType}`,
      `typeName: "${yamlEsc(typeNames[exType] || 'Ejercicio')}"`,
      `title: "${yamlEsc(exTitle)}"`,
      `module: ${moduleNum}`,
      `moduleName: "${yamlEsc(moduleName)}"`,
      `submittedAt: ${nowIso}`,
      `lastUpdated: ${nowIso}`,
      `xpAwarded: ${xpOnSubmit}`,
    ];
    if (repoUrl) fmLines.push(`repoUrl: "${yamlEsc(repoUrl)}"`);
    if (deployUrl) fmLines.push(`deployUrl: "${yamlEsc(deployUrl)}"`);
    fmLines.push('---', '');
    ...
    const bodyParts = [
      ...
      ``,
      `**Módulo:** ${moduleNum} — ${moduleName} · **Clase:** ${classNum} · **Ejercicio:** ${exerciseNum}`,
      ``,
    ];
    if (repoUrl) bodyParts.push(`**Repo:** ${repoUrl}  `);
    if (deployUrl) bodyParts.push(`**Deploy:** ${deployUrl}  `);
    if (repoUrl || deployUrl) bodyParts.push('');
    ...
    bodyParts.push('## Feedback', '', '_(pendiente de revisión automática)_', '');
    ...
    // 1) GET previo para obtener sha si ya existía (re-entrega)
    ...
    try {
      ...
      if (head.ok) {
        ...
        // Si el fichero ya estaba aprobado, no permitir re-entrega
        if (j.content) {
          try {
            const existing = _b64decodeUtf8(j.content);
            const fm = _parseFrontmatter(existing);
            existingStatus = fm.status || null;
          } catch (e) { /* ignore */ }
        }
      } else if (head.status !== 404) {
        // Algo distinto a "no existe": error real
        const txt = await head.text().catch(() => '');
        const cls = _classifyHttpError(head.status, txt);
        recordSubmissionAttempt(classId, exerciseNum, false, head.status, cls.kind, cls.userMsg);
        _renderSubmitErrorBox(exerciseNum, cls.kind, cls.userMsg, head.status);
        buttonEl.textContent = originalText;
        buttonEl.disabled = false;
        return;
      }
    } catch (e) {
      recordSubmissionAttempt(classId, exerciseNum, false, null, 'red_caida', e.message || 'Error de red');
      _renderSubmitErrorBox(exerciseNum, 'red_caida', 'Sin conexión a GitHub. Comprueba tu wifi y reintenta.', null);
      buttonEl.textContent = originalText;
      buttonEl.disabled = false;
      return;
    }
    ...
    if (existingStatus === 'aprobado') {
      _renderSubmitErrorBox(exerciseNum, 'ya_aprobado', 'Este ejercicio ya está aprobado. No se puede re-entregar.', null);
      buttonEl.textContent = originalText;
      buttonEl.disabled = false;
      return;
    }
    ...
    // 2) PUT del fichero
    const putBody = {
      message: `entrega: ${classId} ej${exerciseNum} — ${exTitle}`,
      content: _b64encodeUtf8(fileContent),
    };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const cls = _classifyHttpError(res.status, txt);
        recordSubmissionAttempt(classId, exerciseNum, false, res.status, cls.kind, cls.userMsg);
        _renderSubmitErrorBox(exerciseNum, cls.kind, cls.userMsg, res.status);
        return;
      }

      const result = await res.json();
      const fileUrl = (result.content && result.content.html_url) || `https://github.com/${GITHUB_REPO}/blob/main/${path}`;

      // Guardar referencia local (mantenemos forma compatible con código viejo)
      try {
        const subs = getSubmissions();
        if (!subs[classId]) subs[classId] = {};
        subs[classId][exerciseNum] = {
      ...
      recordSubmissionAttempt(classId, exerciseNum, true, res.status, null, null);

      _showSubmitStatus(exerciseNum,
        `✅ Entregado — <a href="${fileUrl}" target="_blank" style="color:#00e676;">Ver fichero en GitHub</a>` +
        (xpOnSubmit > 0 ? ` (+${xpOnSubmit} XP)` : '') +
        `<br><span style="opacity:0.65;font-size:0.85em;">Revisión automática hoy a las 18:00.</span>`,
        'success');

      _markFormAsSubmitted(container, fileUrl, null, 'pendiente');

    } catch (err) {
      recordSubmissionAttempt(classId, exerciseNum, false, null, 'red_caida', err.message || 'Error desconocido');
      _renderSubmitErrorBox(exerciseNum, 'red_caida', `Error de red al entregar: ${err.message}`, null);
    } finally {
      buttonEl.textContent = originalText;
      buttonEl.disabled = false;
    }
  ...
  function initSubmissionIndicators(classId) {
    ...
    for (const [exNum, data] of Object.entries(classSubs)) {
      ...
      const linkText = 'Ver fichero en GitHub';
      const url = data.url || '#';
      if (data.status === 'aprobado') {
        statusEl.innerHTML = `🏆 Aprobado — <a href="${url}" target="_blank" style="color:#00e676;">${linkText}</a>`;
        statusEl.style.color = '#00e676';
      } else if (data.status === 'revisado') {
        statusEl.innerHTML = `💬 Tienes feedback — <a href="${url}" target="_blank" style="color:#bb86fc;">${linkText}</a>`;
        statusEl.style.color = '#bb86fc';
      } else {
        statusEl.innerHTML = `✅ Entregado — <a href="${url}" target="_blank" style="color:#00e676;">${linkText}</a>`;
        statusEl.style.color = '#00e676';
      }
  ...
  // --- Parser de frontmatter YAML simple ---
  ...
  function _extractFeedback(text) {
    if (!text) return '';
    const idx = text.indexOf('## Feedback');
    if (idx < 0) return '';
    return text.slice(idx + '## Feedback'.length).trim();
  }

  // --- fetchEntregas: enumera con Git Trees (1 llamada) y lee cada fichero con la
  //     Git Blobs API, todo sobre api.github.com. Historia (ver LECCIONES):
  //       1) Contents API carpeta a carpeta = ~17 llamadas/carga -> sin token agota
  //          el limite de 60/hora -> 403 -> "no hay entregas" aunque si las hubiera.
  //       2) raw.githubusercontent.com: bloqueado en muchas redes -> fallo silencioso.
  //       3) actual: Git Blobs API (api.github.com), el host que SI funciona en la red.
  async function fetchEntregas() {
    ...
    // Lanza error claro si la respuesta es por rate limit agotado.
    const checkRate = (res) => {
      if (res.status === 403 && res.headers.get('x-ratelimit-remaining') === '0') {
        throw new Error('RATE_LIMIT: limite de la API de GitHub alcanzado (60/hora sin token). Espera unos minutos y pulsa Actualizar.');
      }
    };

    // 1 llamada API: arbol recursivo de main -> lista de ficheros .md de entregas/
    const treeUrl = `https://api.github.com/repos/${GITHUB_REPO}/git/trees/main?recursive=1&t=${Date.now()}`;
    const treeRes = await fetch(treeUrl, { headers: apiHeaders });
    if (!treeRes.ok) {
      checkRate(treeRes);
      throw new Error('No se pudo leer el repositorio (HTTP ' + treeRes.status + ').');
    }
    const tree = await treeRes.json();
    const nodes = Array.isArray(tree.tree) ? tree.tree : [];
    const fileNodes = nodes.filter(n => n.type === 'blob'
      && typeof n.path === 'string'
      && n.path.startsWith(`${ENTREGAS_DIR}/`)
      && n.path.endsWith('.md'));

    // Contenido de cada fichero via Git Blobs API (api.github.com, devuelve base64).
    // NO usamos raw.githubusercontent.com: ese dominio esta bloqueado en muchas redes
    // corporativas/ISP, y entonces los ficheros fallaban en silencio -> 0 entregas.
    const contents = await Promise.all(fileNodes.map(async (node) => {
      try {
        const r = await fetch(node.url + `?t=${Date.now()}`, { headers: apiHeaders });
        if (!r.ok) { checkRate(r); return null; }
        const j = await r.json();
        if (!j.content) return null;
        return { path: node.path, raw: _b64decodeUtf8(j.content) };
      } catch (e) {
        if (e && String(e.message).startsWith('RATE_LIMIT')) throw e; // propagar rate limit
        return null; // fallo puntual de un fichero: lo saltamos
      }
    }));
    ...
    for (const c of contents) {
      if (!c) continue;
      const fm = _parseFrontmatter(c.raw);
      const feedback = _extractFeedback(c.raw);
      out.push({
        classId: fm.classId || '',
        exerciseNum: fm.exerciseNum || 1,
        type: fm.type || 'B',
        typeName: fm.typeName || '',
        title: fm.title || '',
        status: fm.status || 'pendiente',
        xpAwarded: fm.xpAwarded || 0,
        submittedAt: fm.submittedAt || null,
        lastUpdated: fm.lastUpdated || null,
        fecha: fm.lastUpdated || fm.submittedAt || null,
        repoUrl: fm.repoUrl || '',
        deployUrl: fm.deployUrl || '',
        path: c.path,
        url: `https://github.com/${GITHUB_REPO}/blob/main/${c.path}`,
        feedback,
      });
    }
  ...
  async function pullSubmissionsFromGitHub() {
    try {
      ...
      for (const e of entregas) {
        if (!e.classId || !e.exerciseNum) continue;
        if (!subs[e.classId]) subs[e.classId] = {};
        const existing = subs[e.classId][e.exerciseNum];
        if (!existing || _statusRank(e.status) > _statusRank(existing.status) || existing.path !== e.path) {
          subs[e.classId][e.exerciseNum] = {
            path: e.path,
            url: e.url,
            date: e.submittedAt || e.lastUpdated,
            status: e.status,
      ...
      if (changed) {
        try { localStorage.setItem(SUBMISSIONS_KEY, JSON.stringify(subs)); } catch (ee) { /* ignore */ }
      }
      return changed;
    } catch (e) {
  ...
  async function syncSubmissionStatuses() {
    ...
    await pullSubmissionsFromGitHub();
  ...
  // --- Lectura del log de intentos ---
  function getSubmissionAttempts() {
    try {
      const state = load();
      return Array.isArray(state.submissionAttempts) ? state.submissionAttempts : [];
    } catch (e) { return []; }
  }
...
// Cross-tab sync: si otra pestana actualiza localStorage, recargar estado
