...
const CourseProgress = (() => {
  ...
  // --- fetchEntregas: lee entregas/ via Contents API y devuelve array ---
  async function fetchEntregas() {
    ...
    const out = [];
    try {
      const rootUrl = `https://api.github.com/repos/${GITHUB_REPO}/contents/${ENTREGAS_DIR}?t=${Date.now()}`;
      const rootRes = await fetch(rootUrl, { headers });
      if (rootRes.status === 404) return out;
      if (!rootRes.ok) {
        console.warn('fetchEntregas: HTTP', rootRes.status);
        return out;
      }
      const rootList = await rootRes.json();
      if (!Array.isArray(rootList)) return out;

      const classDirs = rootList.filter(e => e.type === 'dir');
      const filesByClass = await Promise.all(classDirs.map(async (dir) => {
        try {
          const r = await fetch(dir.url, { headers });
          if (!r.ok) return [];
          const arr = await r.json();
          return Array.isArray(arr) ? arr.filter(x => x.type === 'file' && x.name.endsWith('.md')) : [];
        } catch (e) { return []; }
      }));

      const fileItems = [].concat(...filesByClass);
      const contents = await Promise.all(fileItems.map(async (f) => {
        try {
          const r = await fetch(f.url, { headers });
          if (!r.ok) return null;
          const j = await r.json();
          if (!j.content) return null;
          return { meta: j, raw: _b64decodeUtf8(j.content) };
        } catch (e) { return null; }
      }));
      ...
      console.warn('fetchEntregas error:', e);
      return out;
    }
  }
